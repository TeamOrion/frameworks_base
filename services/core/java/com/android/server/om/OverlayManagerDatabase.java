...
class OverlayManagerDatabase {
    ...
    void setState(@NonNull String packageName, int userId, int state, boolean shouldWait) throws BadKeyException {
        DatabaseRow row = select(packageName, userId);
        if (row == null) {
            throw new BadKeyException(packageName, userId);
        }
    ...
    private void notifyOverlayChanged(final OverlayInfo oi, final OverlayInfo oldOi, boolean shouldWait) {
        if (DEBUG) {
            assertNotNull(oi);
            assertNotNull(oldOi);
        }
    ...
    interface ChangeListener {
        ...
        void onOverlayChanged(@NonNull OverlayInfo oi, @NonNull OverlayInfo oldOi, boolean shouldWait);
