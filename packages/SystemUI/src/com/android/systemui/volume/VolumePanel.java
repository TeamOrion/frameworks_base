...
public class VolumePanel extends Handler implements DemoMode {
    ...
    private void toggleRinger(StreamControl sc) {
        if (mAudioManager.getRingerModeInternal() == AudioManager.RINGER_MODE_NORMAL) {
            if (mHasVibrator) {
                ...
                postVolumeChanged(sc.streamType, AudioManager.FLAG_SHOW_UI
    ...
    private void updateTimeoutDelay() {
        mTimeoutDelay = mDemoIcon != 0 ? TIMEOUT_DELAY_EXPANDED
                ...
                : (mActiveStreamType == AudioManager.STREAM_MUSIC &&
                !mExtendedPanelExpanded) ? TIMEOUT_DELAY_SHORT
    ...
    private void updateZenPanelVisible() {
        setZenPanelVisible(mZenModeAvailable && (isNotificationOrRing(
                mActiveStreamType) || mExtendedPanelExpanded));
    }
